...
public class QuickCustomerHandler extends Thread{
	
	
	private void HandleQuick(){
		if(QuickQueue.queue.size() != 0){
			Customer c = (Customer) QuickQueue.queue.poll();
			...
			System.out.println("Sitting down at table 5" + c.getType() + c.getId() + " " + c.getTime() + "minutes");
			try {
			...
			System.out.println("Quick " + c.getType() + c.getId() + " customer leaving...");
